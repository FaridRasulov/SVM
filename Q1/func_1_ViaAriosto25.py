...
def fit(X_train, y_train, kernel, C, kernel_param):
    ...
    tr_time = np.round(time.time() - t, 2)
    NI = sol['iterations']
    ...
    return lambda_star, np.mean(b), NI, obj, tr_time
...
def writetxt(file, C, gamma, kernel, method, obj, conf_mat, val_acc, test_acc, train_acc, NI, tr_time):
    print('\nSaving results in output_question_1.txt')
    output = open(file,"w")
    output.write("This is homework 2: question 1")
    output.write("\nValue of C: " + "%i" % C)
    output.write("\nValue of Gamma: " + "%.3f" % gamma)
    output.write("\nKernel: " + kernel)
    #output.write("\nMethod:" + method)
    #output.write("\nDual objective: " + "%.2f" % obj)
    output.write("\nClassification rate on the validation set: " + "%.2f" % val_acc)
